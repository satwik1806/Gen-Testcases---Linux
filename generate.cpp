...
bool find(vector<pair<int,int>> a, pair<int,int> temp){
...
vector<pair<int,int>> gen_graph(int n, int m, bool weight){
    ...
    for(int i=0;i<m - n + 1;i++){
        int u = randint(0,n-1);
        int v = randint(0,n-1);
        while(v == u or find(edges,{u,v}) or find(edges,{v,u})) v = randint(0,n-1), u = randint(0,n-1), debug() << print('here');
        edges.push_back({u,v});
    }
...
int main(int argc, char* argv[]){
    ...
    for(int i=0;i<n;i++) cout << randint(1,1e6) << " ";
